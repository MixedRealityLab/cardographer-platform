...
<DeckHeader deck={data.revision}>
	...
	{#if data.files.length > 0}
		<a href="{base}/user/decks/{data.revision.deckId}/{data.revision.revision}/zip/{$page.params.file}"
	...
	{/if}
	<form enctype="multipart/form-data" method="post" class="w-6 h-6" use:enhance>
...
<div class="p-6">
	{#if $page.params.file.length > 0}
		<a class="fileItem"
		   href="{base}/user/decks/{$page.params.deckId}/{$page.params.revisionId}/build/{removeLastPathSegment($page.params.file)}">
			<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="w-6 mx-4" alt="back"
			     stroke="currentColor">
	...
	{#each data.files as childFile}
		<div class="flex items-center">
			...
			{/if}
			<button class="text-gray-100 transition-colors duration-500 hover:text-red-500"
