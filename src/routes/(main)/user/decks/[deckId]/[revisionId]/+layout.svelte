<script lang="ts">
	...
	export let data: CardDeckRevisionSummary
		...
		<div class="flex-1">
			{#if data}
				<a class="block hover:text-blue-700 transition-colors duration-500 inline-flex items-center gap-2"
				   href="{base}/user/decks/{data.deckId}/{data.revision}/revisions"
				   title="Select Revision">{data.deckName}
					{#if data.deckName.toLowerCase().indexOf('deck') === -1}
						Deck
					{/if}
					<span class="opacity-50">v{data.revision}</span>
					{#if data.revisionName}
						<span class="opacity-50 font-normal">{data.revisionName}</span>
					{/if}
