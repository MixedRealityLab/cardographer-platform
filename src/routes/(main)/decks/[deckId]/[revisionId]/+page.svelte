...
<form class="p-6 flex flex-col gap-4" method="post" use:enhance>
	<label>
		<span>Title</span>
		<input name="deckName" bind:value={data.deckName} class="block w-full" required type="text" disabled={!data.isOwnedByUser}/>
	</label>
	<label>
	...
	<label>
		...
		<input name="revisionName" bind:value={data.revisionName} class="block w-full" type="text" disabled={!data.isOwnedByUser}/>
	</label>
	<label>
	...
	<div class="flex flex-wrap justify-center gap-4 py-1">
		...
		<label class="flex items-center gap-2">
			<input name="isPublic" bind:checked={data.isPublic} class="form-checkbox" type="checkbox" disabled={!data.isOwnedByUser}>
			<span>Public</span>
		</label>
		<label class="flex items-center gap-2">
			<input name="isLocked" bind:checked={data.isLocked} class="form-checkbox" type="checkbox" disabled={!data.isOwnedByUser}>
			<span>Locked</span>
		</label>
		<label class="flex items-center gap-2">
			<input name="isTemplate" bind:checked={data.isTemplate} class="form-checkbox" type="checkbox" disabled={!data.isOwnedByUser}>
			<span>Template</span>
		</label>
		{#if data.build}
			<label class="flex items-center">
				<input name="buildDisabled" type="checkbox" class="form-checkbox" bind:checked={data.build.isDisabled} disabled={!data.isOwnedByUser}>
