...
async function handleSubmit() {
	...
	if (res.ok) {
		...
		if (output.error) {
			error = output.error;
			revision.build.status = 'failed';
		} else {
			message = 'OK';
			revision.build.status = 'built';
...
<div class="grid grid-cols-1 gap-2 text-sm">
	<label class="block">
		...
		<input readonly class="mt-1 block w-full" required type="text" bind:value="{revision.build.builderName}" />
	</label>
        <label class="inline">
	...
	<label class="block">
		...
		<textarea readonly rows="3" class="mt-1 block w-full" type="text" bind:value="{buildMessages}"></textarea>
	</label>
...
{/if}
        <input disabled={working || revision.build.isDisabled || revision?.isLocked} class="rounded-md mt-1 block w-full bg-gray-300 py-2" class:text-gray-400="{working || revision.build.isDisabled || revision?.isLocked}" type='submit' value='Build'>
</div>
</form>
{:else}
{#if revision}
