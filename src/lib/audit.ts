...
export async function exportAuditAsCsv(): Promise<string> {
    ...
    for (const user of users) {
        ...
        let audit: UserAudit = {
            ...
            publicRevisions: await db.collection<CardDeckRevision>("CardDeckRevisions").count({quotaUser:email, isPublic:true}),
            publicSessions: await db.collection<Session>("Sessions").count({quotaUser:email, isPublic:true}),
            publicSnapshots: await db.collection<SessionSnapshot>("SessionSnapshots").count({quotaUser:email, isPublic:true}),
...
export async function checkAndFixDb(defaultEmail:string, fix:boolean = true): Promise<void> {
    ...
    for (let revision of revisions) {
        ...
        if (!deck) {
            ...
            let max = revisions.filter((r) => r.deckId == revision.deck_id).reduce((m, r) => Math.max(m, r.revision), 0)
            deck = {
                _id: revision.deckId,
                isPublic: false,
                owners: [quotaUser],
        ...
        if (typeof(revision.diskSizeK) != 'number') {
            ...
            if (fix) {
                console.log(`Fix revision ${revision.deckId}/${revision.revision} diskSizeK as ${deck.diskSizeK}`)
                await db.collection<CardDeckRevision>("CardDeckRevisions").updateOne(
                    { _id: revision._id },
                    {
                ...
                console.log(`revision ${revision.deckId}/${revision.revision} has no diskSizeK - fix as ${deck.diskSizeK}?`)
            }
        }
    }
    // check all sessions
    ...
    // check all emails...
    let users = await db.collection<User>("Users").find({},
        {   projection: {
                _id: true, email: true,
        }}).toArray()
